...
shared.htmlColorToRgba = function (htmlColor, opacity) {
    var hColor = shared.colorTable[htmlColor];
    if (hColor) {
        htmlColor = hColor;
    }
    ...
    if (htmlColor[0] === "#") {
        ...
        return [parseInt(result[1], 16), parseInt(result[2], 16), parseInt(result[3], 16), shared.isEmpty(opacity) ? 255 : opacity];
    } else {
        if (shared.isEmpty(opacity)) {
            opacity = 1.0;
        }
        var m = htmlColor.split(",");
        return [parseInt(m[0].split("(")[1]), parseInt(m[1]), parseInt(m[2]), !shared.isEmpty(m[3]) ? parseFloat(m[3]) : opacity];
    }
};

shared.isEmpty = function (v) {
    return v == null;
};
    ...
    $.ajax({
        ...
        $(xml).find("ENTRY").each(function () {
            var code = $(this).find('Code').text(),
                ...
                htmlColor = $(this).find('Color > Html').text();
            if (htmlColor) {
                var c = shared.htmlColorToRgba(htmlColor)
                palette[parseInt(code)] = { 'partRed': c[0], 'partGreen': c[1], 'partBlue': c[2] };
            } else {
...
shared.getCookie = function (name) {
